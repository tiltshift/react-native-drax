...
const DraxListComponent = ({ data, style, wrapperStyles, itemStyles, renderItemContent, renderItemHoverContent, onItemReorder, id: idProp, reorderable: reorderableProp, onDragPositionChanged, onDragStart, onDragEnd, ...props }, flatListRef) => {
    ...
    const renderItem = react_1.useCallback((info) => {
        ...
        return (react_1.default.createElement(DraxView_1.DraxView, Object.assign({ style: [
                style,
                { transform: getShiftTransform(originalIndex) },
            ], draggingStyle: draggingStyle, dragReleasedStyle: dragReleasedStyle }, otherStyleProps, { payload: { index, originalIndex }, onDragStart: (evt) => {
                if (onDragStart)
                    onDragStart(evt);
            ...
            }, renderContent: (props) => renderItemContent(info, props), renderHoverContent: renderItemHoverContent &&
                ((props) => renderItemHoverContent(info, props)), longPressDelay: params_1.defaultListItemLongPressDelay })));
    }, [
        originalIndexes,
        getShiftTransform,
        setDraggedItem,
    ...
    const handleInternalDragEnd = react_1.useCallback((dragged, receiver) => {
        ...
        if (reorderable) {
            ...
            const fromPayload = dragged && dragged.parentId === id
                ? dragged.payload
                : undefined;
            const toPayload = curToPayload.current ||
    ...
    const onMonitorDragOver = react_1.useCallback(({ dragged, receiver, monitorOffsetRatio, dragAbsolutePosition, }) => {
        ...
        const dragIsWithinList = dragAbsolutePosition.y <= bottom &&
            dragAbsolutePosition.y >= top &&
            dragAbsolutePosition.x <= right &&
            dragAbsolutePosition.x >= left;
        ...
        if (reorderable && dragged.parentId === id && dragIsWithinList) {
            ...
            const toPayload = receiver?.parentId === id ? receiver.payload : fromPayload;
...
exports.DraxList = react_1.default.forwardRef(DraxListComponent);
